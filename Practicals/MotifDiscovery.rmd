...
and load the function you will find here as [readfastafile.R](https://raw.githubusercontent.com/christoforos-nikolaou/MolBioMedClass/master/Rfunctions/readfastafile.R) as well as [seqMotif.R](https://raw.githubusercontent.com/christoforos-nikolaou/MolBioMedClass/master/Rfunctions/seqMotif.R) which you will find at the respective links
...
At this point, we will read the GATA sequences file using the readfastafile function and store them in a sequence array called gata. You can download the file from [here](https://raw.githubusercontent.com/christoforos-nikolaou/MolBioMedClass/master/Datasets/gata.fa). In my computer, this is already stored in a different folder so I run:  
```{r}
...
We now have the sequences stored in an R data structure. Our next goal is to extract a consensus sequence from the set of sequences in our collection. Remember that the consensus sequence corresponds to the most common residue extracted by position. You can extract the consensus sequence using the namesake function [consensus.R](https://raw.githubusercontent.com/christoforos-nikolaou/MolBioMedClass/master/Rfunctions/consensus.R) as shown below:
```{r}
...
To better understand the strength of a probabilistic representation of a DNA sequence, i.e., how characteristic the pattern it contains is, it is good to compare it with a set of random sequences. You can do all the above steps on a set of random sequences of the same length (6 nucleotides) which you will find in the [random.fa](https://raw.githubusercontent.com/christoforos-nikolaou/MolBioMedClass/master/Datasets/random.fa) file.  
...
Having created the PSSM, you are now in the position to search for this specific motif in a DNA sequence. You can use this sequence  [test1.fa](https://raw.githubusercontent.com/christoforos-nikolaou/MolBioMedClass/master/Datasets/test1.fa) as an example. Read it according to what we have done already.
```{r}
targetseq<-readfastafile("../Datasets/test1.fa")
```

And now also download the PSSM-Searching function from [pssmSearch.R](https://raw.githubusercontent.com/christoforos-nikolaou/MolBioMedClass/master/Rfunctions/pssmSearch.R). Once loaded
```{r}
