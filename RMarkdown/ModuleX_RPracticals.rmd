...
```{r}
setwd("C:/Users/user/Dropbox/github/MolBioMedClass/RMarkdown")
```
...
```{R}
boxplot(gcGenes$GC, gcRegSeq$GC, lwd=2, col="blue", names=c("Genes", "Reg Seqs"), main="Yeast GC%", las=1, xlab="Genome Partition", cex.main=2, cex.axis=1.3, cex.lab=1.5)
```
...
c. Because the 95% confidence interval of the test is [0.958261, 3.97442] which **does contain the basal value of 1**. 
...
You see that the dataset contains a large number of variables related to the names of the genes, their positions in the genome, methylation and expression values for three different time points, as well as information on the groups they form in terms of their expression patterns. We will focus on one particular timepoint (the latest one) and obtain the mean values of methylation (for the body of the gene) and the expression score for that one only. 
The desired variables are: GeneName, LateExp, meanLate_gb (corresponding to columns 1, 4 and 13 respectively).

```{r}
expmethdataLate <- expmethdata[,c(1,4,13)]
...
A trained eye may spot some positive correlation. We can try to calculate this with a suitable function named cor().

```{r}
cor(expmethdataLate[,2:3])
```

It turns out our eyes were mistaken and the correlation was actually negative. Well, not exactly. The value is undermined by a considerable proportion of zero values for methylation. We can remove them and recalculate.

```{r}
nzExpMethDataLate <- expmethdataLate[which(expmethdataLate$meanLate_gb!=0),]
cor(nzExpMethDataLate[,2:3])
```

Now, we see that the positive correlation we spotted is indeed there, even if only marginal.

##### Additional things to consider

Correlation is tightly associated with the concept of regression, which is, in turn, the introductory concept to modeling data. 
We will come back to this (hopefully) next year.
...
We can now apply PCA on this subset and check the outcome. Notice that we only use columns 2-936 since the first and last carry label values.
...
We can plot the outcome of the PCA values like so:
...
In the example shown above we used some additional layering in the plotting functions. This is based on the ggplot2 library, whose details we cannot afford to go into here. You can check out more yourselves [here](https://ggplot2.tidyverse.org/).

Our output shows some degree of underlying batch effect in our dataset. That is our data points are not perfectly distributed according to condition and some conditions appear to be closer to each other. We can test this further by attempting to cluster our data.

We will use kmeans with k=6, since we know that we have 6 different conditions
...
Once the data are grouped in 6 clusters we can compare the cluster distribution with the actual underlying conditions. We can do this with table() which will provide a 6X6 contingency matrix. A contingency matrix is a table showing the coincidence between two types of categorical groupings. Here it is going to be clusters vs conditions.
...
The desired case for such a table is to have values clustered on the diagonal of the matrix, as this will imply a 1:1 relationship of the two categorical variables. We see that, while some of the combinations are indeed found on the diagonal, there are also quite a few off-diagonal enrichments.

This suggests that our defined clusters correspond to something other than the conditions. 

#### 2.3 Sequence motifs

As a last task we will perform a motif building and search analysis using a set of defined motif instances and custom-made R functions.

Our goal is to create a PWM with a set of instances of the GATA transcription factor, transform it into PSSM and then search it against a whole genome sequence.

1. You may start by downloading a set of GATA binding sequences that you will find [here](https://github.com/christoforos-nikolaou/MolBioMedClass/blob/master/Datasets/gata.fa).

2. Once this is done, you can download the target sequence against which you will search the PWM [here](https://github.com/christoforos-nikolaou/MolBioMedClass/blob/master/Datasets/test1.fa).

3. In order to build the PWM you need to read the GATA sequences using [this custom function](https://github.com/christoforos-nikolaou/MolBioMedClass/blob/master/Rfunctions/readfastafile.R) and then create the matrix with the [PWM function](https://github.com/christoforos-nikolaou/MolBioMedClass/blob/master/Rfunctions/seqMotif.R). 
...
```{r, eval=F, echo=T}
source("../Rfunctions/readfastafile.R")
gataseqs<-readfastafile("../Datasets/gata.fa")
```

and then obtain the PWM with:
```{r, eval=F, echo=T}
source("../Rfunctions/seqMotif.R")
gataPWM<-seqMotif(gataseqs, drawmotif=F)
```

We can visualize the motif with the same function, changing the parameter drawmotif to TRUE

```{r}
seqMotif(gataseqs, drawmotif=TRUE)
```
...
4. The transformation of the PWM to PSSM requires an additional random sequence collection, which you can find [here](https://github.com/christoforos-nikolaou/MolBioMedClass/blob/master/Datasets/random.fa). You can then create the PSSM by taking the log2(ratio) of the two PWMs (one from the GATA sequences over the one from the random sequence).

```{r}
randomseqs<-readfastafile("../Datasets/random.fa")
seqMotif(randomseqs, drawmotif=T)
```

```{r}
randomPWM<-seqMotif(randomseqs, drawmotif=F)
```

Create the PSSM by log2(PWM1/PWM2)

```{r}
pssm<-log2(gataPWM/randomPWM)
pssm
```



5. In the next step you will use a [PSSMSearch Function](https://github.com/christoforos-nikolaou/MolBioMedClass/blob/master/Rfunctions/pssmSearch.R) to search for the motif in the longer target sequence. Assuming you have created the PSSM into the table called $pssm$, the list of commands below should work:
...
```{r}
pssmHits <- pssmSearch(pssm, targetseq, threshold=0.5)
myHits <- vector(mode="character", length=length(pssmHits[,1]))
for(i in 1:length(pssmHits[,1])){
  position <- pssmHits[i,1]
  myHits[i] <- substr(targetseq[[1]], position, position+6)
}
myHits
```

```{r}
write.table(myHits, file="../Datasets/PSSMHits.fa", row.names = F, quote=F, col.names = F)
```
