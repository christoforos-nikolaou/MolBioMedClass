...
library(multcomp)
data<-read.delim("Exp1234_raw_common18704genes_antiTNF.tsv", header=T, sep="\t")
boxplot(data[,2:67])
genes=data$Gene
...
write.table(all, file="Supplement/temp", row.names = F, sep="\t", quote=F, col.names = T)
# read from temp
results<-read.delim("Supplement/temp", header=T, sep="\t")
